/**
 * Telemetry readers for the parent App-Activity view.
 *
 * Consumer apps (helena-spelling, helena-states, helena-math) write
 * gameplay telemetry into `module_overrides.<module>.*` and the parent
 * pastes the re-exported profile into the dashboard to inspect it.
 *
 * The schema is defined informally in each consumer's profile.js / store
 * (cross-mirrored). The dashboard cannot trust that all four implementations
 * are bug-free, so these readers are defensive — every field falls back to a
 * sensible default rather than throwing or rejecting the whole module.
 *
 * Shape (cross-module contract):
 *   module_overrides.<key> = {
 *     followed:           { [mode]: count }
 *     overrode:           { [mode]: count }
 *     last_launches:      string[]   // most-recent first, capped at ~12
 *     last_override_streak: number  // 0 after a follow
 *   }
 */
...
function coerceCountMap(v: unknown): Record<string, number> {
	...
	for (const [k, val] of Object.entries(v)) {
		// Counts are non-negative integers. Drop anything that isn't.
...
/**
 * Read one module's telemetry. Returns null when the module slot is missing
 * OR when every field is empty (no point rendering a zero-state card from
 * stray writes).
 */
...
/** Modes sorted by total launches descending. */
